...
//reference for cost function
double ref_cte = 0.0;
double ref_epsi = 0.0;

double ref_v = 26; //26 m/s
...
public:
    ...
    void operator()(ADvector& fg, const ADvector& vars) {
        ...
        double weight_a = 20.0;
        double weight_delta_step = 600.0;

        // cost based on the reference state.
        for (int t = 0; t < N; t++){
            fg[0] += CppAD::pow((vars[t + cte_start] - ref_cte), 2);
            fg[0] += CppAD::pow((vars[t + epsi_start] - ref_epsi), 2);
            fg[0] += CppAD::pow((vars[t + v_start] - ref_v), 2);
        }
        ...
        // constraint required by the motion equation
        for (int t = 1; t < N; t++){
            // state at time t+1 
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    for (int i = delta_start; i < a_start; i++) {
        vars_lowerbound[i] = -0.436332; //-25º in radians
        vars_upperbound[i] = 0.436332;  //25º
    }
    ...
    result_var.push_back(solution.x[delta_start]);
    result_var.push_back(solution.x[a_start]);
